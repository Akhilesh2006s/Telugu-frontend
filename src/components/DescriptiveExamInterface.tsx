  ...
  useEffect(() => {
    if (examStatus === 'active' && isStarted && exam.attemptId) {
      ...
      const updateTimer = async () => {
        try {
          const response = await fetch(`https://backend-production-7e4df.up.railway.app/api/exam-attempts/status/${exam.id}`, {
            headers: {
    ...
    try {
      ...
      for (let i = 0; i < exam.descriptiveQuestions.length; i++) {
        ...
        if (file) {
          ...
          const uploadResponse = await fetch('https://backend-production-7e4df.up.railway.app/api/uploads/upload', {
            method: 'POST',
            headers: {
          ...
          if (uploadResponse.ok) {
            ...
            submission = {
              ...submission,
              pdfUrl: `https://backend-production-7e4df.up.railway.app${uploadResult.data.url}`,
